...
void display(float drawScale, boolean drawName, boolean drawPoints, boolean drawBox, boolean drawHistory, color fColor)
{
...
fill(fColor);
    ...
    textSize(60);
    text(blobNumber+"",(float)center.getX()*drawScale,((float)center.getY()*drawScale)+0);
    if(drawName)
    {
    textSize(20);
     text(name+"",(float)center.getX()*drawScale+50,((float)center.getY()*drawScale)+0);
    }
    textSize(20);
